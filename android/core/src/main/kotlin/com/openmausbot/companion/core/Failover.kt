...
object ConnectionAdvice {
    ...
    fun shouldTryAnotherRoute(error: Throwable): Boolean {
        ...
        return status in PROXY_GATEWAY_STATUSES || status in TUNNEL_GATEWAY_STATUSES
    }

    /** The HTTP status this failure should be reported as, when a gateway produced it. */
    fun gatewayStatus(error: Throwable): Int? = statusCode(error)?.takeIf {
        it in PROXY_GATEWAY_STATUSES || it in TUNNEL_GATEWAY_STATUSES
    }
